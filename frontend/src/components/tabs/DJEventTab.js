...
const DJEventTab = () => {
  ...
  const [djEventData, setDjEventData] = useState(null);
  ...
  const getServicesByCategory = async (category) => {
    ...
    try {
      ...
      if (response.data.success) {
        setDjEventData(response.data.data);
      } else {
        setDjEventData(null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if(!djEventData){
      getServicesByCategory('DJ Events');
    }
  }, [])
            ...
            <div className="flex flex-col gap-1">
              {data.links &&
                data.links.map((item) => (
                  <Link
                    key={item.id}
                    to={`/s/dj-events/${data.heading.replace(/\s+/g, "-").toLowerCase()}/${item.title.replace(/\s+/g, "-").toLowerCase()}`}
