...
const Login = () => {
    ...
    const verificationOtp = async (userId) => {
        ...
        try{
            ...
            const url = `${baseUrl}/admin/login-otp`;
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const verifyOtp = async() => {
        try{
            ...
            if((otpCode === serverOpt) && (userToken !== '')){
                localStorage.setItem('VideekAdmin', userToken);
                await AuthAdmin();
        ...
        <div className='w-[500px] flex flex-col text-white bg-[#111] py-6 px-8 max-sm:p-4 rounded-xl gap-6 max-sm:gap-4 max-md:w-[400px] max-sm:w-full'>
            ...
            <div className='w-full flex flex-col gap-6 max-md:gap-3'>
                ...
                {!isVerifyOtp && (<div className='w-full flex items-center justify-start gap-2'>
                    ...
                    <p className='text-md max-md:text-sm font-semibold text-blue-600 cursor-pointer transition duration-200 ease-in hover:underline' onClick={() => navigate('/signup')}>Create account</p>
                </div>)}
            </div>
        </div>
    </div>
