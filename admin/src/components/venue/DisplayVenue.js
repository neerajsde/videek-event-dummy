...
const DisplayVenue = () => {
  ...
  const getAllServicesCategory = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/all/venue`
      );
  ...
  function deleteHandler(data){
    setIsActiveConformation({isActive: true, data: data, name:`Venue - ${data.category}, Name: ${data.name}`, tab:'venue', response:false});
  }

  useEffect(() => {
    if(((isActiveConformation.tab === 'venue') && isActiveConformation.response) && venueData){
      const filterData = venueData.filter((item) => item._id !== isActiveConformation.data._id);
      setVenueData(filterData);
    }
                ...
                {venueData &&
                  venueData.map((item, idx) => (
                    <tr key={item.id} className="border-b border-gray-300">
                        ...
                        <td className="py-2 px-4 text-center"><Link to={`tel:${item.phone}`}>{item.phone}</Link></td>
