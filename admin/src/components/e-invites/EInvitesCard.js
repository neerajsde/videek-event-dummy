...
import DisplayEInvitation from "./DisplayEInvitation";
import AddEInvites from "./AddEInvites";
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      <div className="w-full flex items-center justify-between bg-[#111] p-3 rounded-sm !border !border-[#333]">
        ...
        >
          <IoHome className="text-xl" /> Invitations Crads
        </button>
      ...
      <div className="w-full bg-[#111] rounded-sm !border !border-[#333]">
        {currentSection === "/" && <DisplayEInvitation />}
        {currentSection === "add" && <AddEInvites />}
      </div>
    </div>
  );
};

export default EInvitesCard;